...
function fieldValidate(field) {
  ...
  switch (field.name) {
    case "phone":
      elValidate = setFieldColor(
        field,
        field.value.length >= 7 && field.value.length < 20 && rePhone.test(field.value)
      );
      break;
    case "email":
    ...
    default:
      elValidate = setFieldColor(
        field,
        field.value.length > 2 && field.value.length < 50 && reFullname.test(field.value)
      );
      break;
  }
...
function formValidate(formBtn, formInput) {
  ...
  document.querySelectorAll(formInput).forEach((input, i) => {
    input.addEventListener("input", () => {
      ...
      document.querySelector(formBtn).disabled = !(
        checkResults.length === 4 && !checkResults.includes(false)
      );
    });
...
formValidate(".form-registration__btn", ".form-registration__input");
